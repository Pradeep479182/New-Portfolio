...
export default function ContactModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const mainTemplateId = import.meta.env.VITE_EMAILJS_TEMPLATE_ID || 'YOUR_TEMPLATE_ID';
      ...
      const response = await emailjs.send(serviceId, mainTemplateId, templateParams);
      ...
      if (response.status === 200) {
        if (autoReplyTemplateId) {
          const autoReplyParams = {
            ...
            reply_subject: 'Thanks for your feedback!',
            reply_message:
              'Thanks for reaching out! I received your message and will get back to you shortly.',
          };

          emailjs.send(serviceId, autoReplyTemplateId, autoReplyParams).catch((replyError) => {
